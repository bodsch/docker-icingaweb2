...
ICINGAWEB_REPORTING=${ICINGAWEB_REPORTING:-"true"}
...
check() {
  ...
  if [[ "${ICINGAWEB_REPORTING}" = "false" ]]
  then
    log_info "    reporting support is disabled"

    disable_module reporting
    exit 0
  fi
}
...
create_database() {
  ...
  then
    ...
    (
      ...
      echo "GRANT SELECT, INSERT, UPDATE, DELETE, DROP, CREATE VIEW, INDEX, EXECUTE ON ${REPORTING_DATABASE_NAME}.* TO '${REPORTING_DATABASE_USER}'@'%';"
      echo "FLUSH PRIVILEGES;"
      echo "quit"
    ) | mysql ${MYSQL_OPTS}
